...
class RepositoryFirestoreAuth(application: Application): Repository {
    ...
    override suspend fun getTags(): List<String> = mutableSetOf<String>().also { set ->
        getCollection()
            .whereEqualTo(TaskDoc.user, getCurrentUser())
            .get(getSource()).await().forEach { queryDocumentSnapshot ->
    ...
    override suspend fun update(task: Task) {
        getCollection()
            ...
            .let { querySnapshot ->
                ...
                querySnapshot.first().reference.set(TaskFirestore.fromTask(task, getCurrentUser())) //set atualiza caso exista e adiciona caso não exista (precisa de id, ao contrário de add)
            }
    }
